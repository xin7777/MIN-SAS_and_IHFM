...
try:
    ...
    try:
        command_to_execute = "pip3 install pymysql"
...
class S_Assessment():
    ...
    def __init__(self):
        ...
        self.db = pymysql.connect(host="localhost", user="root", password=self.conf.get("Password","Mysql"), port=int(self.conf.get("Port","mysql")))
    ...
    def read_data(self, mins):
        ...
        try:
            print("==========\nsql: ",sql,"============")
            self.db = pymysql.connect(host="localhost", user="root", password=self.conf.get("Password","Mysql"), port=3306)
            self.cursor = self.db.cursor()
            self.cursor.execute("use ossec;")
...
#主机态势威胁值
    def run(self, mins):
        ...
        db.authenticate(self.conf.get("Password","Mongo"),self.conf.get("Password","Mongo"))
        collection_name = 'SA_host_value'
        collection = db[collection_name]
        while True:
            result = self.read_data(mins)
            #ans = {"value":result, "time":time.strftime("%Y-%m-%d %H:%M:%S")}
            ans = {"value":result, "time":int(time.time() * 1000), "htime":time.strftime("%Y-%m-%d %H:%M:%S")}
