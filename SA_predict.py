...
def load_data(client, name):
    ...
    #print("CONFGIG:============",conf.get("Port","mongo"),"+======")
    db.authenticate(conf.get("Password","mongo"), conf.get("Password","Mongo"))
    datas = list(collection.find().limit(6).sort([('_id', -1)]))
    if len(datas) < 6 and name == "SA_host_value" :
        print("填充主机态势值数据")
        length = len(datas)
        for i in range(6-length):
            # 注意：这里必须使用深复制，否则指向同一地址，会修改掉原数据
            new_data = copy.deepcopy(datas)[length-1]
            new_data["time"] = new_data["time"] - 3 * 60 * 1000 * (i+1)
...
'''
def connect():
    # modified at 2021/1/12 fix the warning: "MongoClient opened before fork. Create MongoClient only"
...
def predict(trainX, trainY, time_list, client, name, weight):
    ...
    for i in range(len(trainY)):
        ...
        if trainY[i] == 0:
            ...
            temp_value = trainY[i] + entropy * weight
        # print temp_value
        data = {'value': temp_value, 'time': time_list[i], 'htime':strftime("%Y-%m-%d %H:%M:%S")}
        print(data)
...
def weigh(data):
    #loaded_model = pickle.load(open('SA_model.model', 'rb'), encoding='bytes')
