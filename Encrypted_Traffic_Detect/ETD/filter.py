...
#Dictionaries of top 1 million visited sites in the world, ciphersuites, supported groups, and extensions
benign_urls = {}

def setBenignUrls(value):
    global benign_urls
    benign_urls = value
...
copy = ["Src_Port", "Dst_Port", "Bytes_in", "Bytes_out", "Pkts_in", "Pkts_out", "entropy", "byte_dist_std", "byte_dist_mn", "num_of_exts"]


def setCipher(json_obj, list):
...
def setExtensions(json_obj, list):
    json_obj["num_of_exts"] = 0
    for key, value in extensions_dict.items():
        json_obj[value] = 0
        for ext in list:
            if key in ext:
                json_obj[value] = 1
                json_obj["num_of_exts"] += 1


def setSGandECPoints(json_obj, list):
    for ext in list:
...
def filterJsons(mal_or_ben, jsons):
    try:
        for tls_json in jsons:
...
def featuresToCSV(mal_or_ben, json_obj):
    file_name = ""
    if mal_or_ben == "ben":
        file_name = "benign_flows2.csv"
    elif mal_or_ben == "mal":
        file_name = "malicious_flows2.csv"
...
def supplement():
    length = len(supported_groups_dict)
    if length < 26:
        diff = 26 - length
        for i in range(diff):
            supported_groups_dict[i] = "sg_" + str(length + i)


def cleanJsons(jsons):
    for json_obj in jsons:
        cleaned_list = [json_obj['sp'], json_obj['dp'], json_obj['bytes_in'], json_obj['bytes_out'],
                        json_obj['num_pkts_in'], json_obj['num_pkts_out'], json_obj['entropy'],
                        json_obj['byte_dist_std'],
                        json_obj['byte_dist_mean']]
        for i in range(9):
            json_obj[copy[i]] = cleaned_list[i]
        json_obj["ec_pts_1"] = 0
        cs_binary_vector = getCipherVector(json_obj['tls']['cs'])
        sg_binary_vector, ec_binary_vector = getSGandECPointsVector(json_obj['tls']['c_extensions'])
        num_of_extensions, ext_binary_vector = getExtensionsVector(json_obj['tls']['c_extensions'])
        setJsonAttributes(json_obj, cs_binary_vector, cipher_suites_dict)
        setJsonAttributes1(json_obj, sg_binary_vector, supported_groups_dict)
        setJsonAttributes1(json_obj, ec_binary_vector, ec_point_formats_dict)
        setJsonAttributes1(json_obj, ext_binary_vector, extensions_dict)
        json_obj["num_of_exts"] = num_of_extensions
    ...
    for json_obj in jsons:
        for key in json_obj.keys():
            if key == "ec_pts_1":
                continue
            if key not in list_of_features:
                json_obj.pop(key)


def setJsonAttributes(json_obj, vector, dict):
    keys = dict.keys()
    print (len(vector))
    for i in range(len(vector)):
        json_obj[keys[i]] = vector[i]

def setJsonAttributes1(json_obj, vector, dict):
    values = dict.values()
    for i in range(len(vector)):
        json_obj[values[i]] = vector[i]

def getFilteredJsons(mal_or_ben, jsons):
    filterJsons(mal_or_ben, jsons)
    supplement()
    cleanJsons(benign_jsons)
    return benign_jsons
