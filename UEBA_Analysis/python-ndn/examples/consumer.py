...
async def main():
    try:
        ...
        name = Name.from_str('/MIN-VPN/testflow/SAS') + [Component.from_timestamp(timestamp)]
        print(f'Sending Interest {Name.to_str(name)}, {InterestParam(must_be_fresh=True, lifetime=6000)}')
        data_name, meta_info, content = await app.express_interest(
            name, must_be_fresh=True, can_be_prefix=False, lifetime=6000, app_param='hello world'.encode('utf-8'))
