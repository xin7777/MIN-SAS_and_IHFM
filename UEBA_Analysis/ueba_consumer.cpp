...
public:
    ...
    {
        Interest interest(Name("/MIN-VPN/testflow/SAS"));
        ...
        std::cout << "Sending " << interest1.getName().toUri() << std::endl;
...
int main(int argc, char** argv)
{
    ...
    char buf[BUFSIZE];
    try {
        int char_read = fgets(buf, BUFSIZE, stdin);
        printf("param: %s\n", char_read);
        string param(buf, char_read);
        consumer.run(param);
    }
