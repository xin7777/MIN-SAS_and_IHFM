...
def train():
    ...
    pickle.dump(clf, open("urlDetect3.model", 'wb'), protocol=3)
...
def detectBatch(data):
    try:
        ...
        for request in data:
            request = re.sub('"', '', request)
            urls.append(parse_http_packet(request))
        ...
        for ans in result:  # abnormal request [-1 1], reversed result means we will generate res in tuples 
            if eval(data[urlCounter])['data'][0:6] == '\u0005' or eval(data[urlCounter])['data'][
                                                                  0:6] == '\u0006':  # to generate tuples in sequent way
