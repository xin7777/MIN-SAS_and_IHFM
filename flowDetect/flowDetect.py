...
whitelist = ["223.73.111.93", "58.60.1.47", "121.15.171.91"]


def InWhiteList(ipaddr):
...
def ddos_detect(data):
    ...
    for i in range(len(data)):
        ...
        # block ips of NDN servers
        if not InWhiteList(temp["Source IP"]) and not InWhiteList(temp["Dest IP"]):
    ...
    print(df.loc[:, ['Source IP', 'Attack Length']])
    ...
    for check in prediction:
        if check == 1 or df.loc[ddos_counter, 'Attack Length'] == 1:  # change to 0 to force ddos attack
            if num1 == 0:
...
def nmap_detect(data):
    ...
    print(X.loc[:, ['Source IP', 'Attack Length']])
    ...
    for check in prediction:
        if check == 1 or df.loc[nmap_counter, 'Attack Length'] == 1:  # change to 0 to force ddos attack
            if num2 == 0:
